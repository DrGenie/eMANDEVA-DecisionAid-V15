...
## Step 4. Copy the Worker URL into the frontend

1. Open `assistant.js` in the tool repo.
...
3. Replace it with your deployed URL, keeping the `/api/emandeval-chat` path:

   ```
   const CHATBOT_WORKER_URL = "https://emandeval-chat.YOUR-SUBDOMAIN.workers.dev/api/emandeval-chat";
   ```

4. Commit and push to GitHub so GitHub Pages serves the updated file:

   ```
   git add assistant.js
   git commit -m "Connect Policy Assistant to deployed Worker"
   git push
   ```
